...
def asserts_update_historical(
    exchange: str, pair: str, interval: str, start_ts: int, end_ts: int
):
    client = clients(
        exchange=exchange,
    ...
    earliest_start = client._get_earliest_timestamp(pair=pair, interval=interval)
    ...
    df = client.get_historical_data(
        pair=pair, interval=interval, start_ts=real_start, end_ts=end_ts
    )

    up_df = client.update_historical(pair=pair, interval=interval, current_df=df)
    ...
    assert (
        up_df["open_time_difference"].max() == time_milli
    ), "Candle interval is wrong for open_time"
    assert (
        up_df["close_time_difference"].max() == time_milli
    ), "Candle interval is wrong for close_time"

    assert (
        up_df["open_time_difference"].min() == up_df["open_time_difference"].max()
    ), "Time series not respected"
    assert (
        up_df["close_time_difference"].min() == up_df["close_time_difference"].max()
    ), "Time series not respected"
